...
export default defineComponent({
  ...
  setup(props: IProps, ctx){
    ...
    const inViewReport = function (windowWidth=0,windowHeight=0, {x=0, y=0}){
    ...
    let imgRef : Ref = ref(null);
    const onScroll = utils.debounce( function (){
      if(imgRef.value != null){
        let clientRect = imgRef.value.getBoundingClientRect();
        const isViewReport = inViewReport(windowWidth,windowHeight, clientRect)
        if(isViewReport){
