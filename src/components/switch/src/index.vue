...
export default defineComponent({
  ...
  setup(props: IFeSwitchProps, { emit, attrs, slots }) {
    const elForm = inject(elFormKey, {} as ElFormContext);
    const elFormItem = inject(elFormItemKey, {} as ElFormItemContext);
    const input = ref(null);
    const core = ref(null);

    const scope = "FeSwitch";
    ...
    if (!~[props.activeValue, props.inactiveValue].indexOf(actualValue.value)) {
      ...
      emit("update:change", props.inactiveValue);
      emit("update:input", props.inactiveValue);
    }
    ...
    watch(checked, () => {
      ...
      if (props.validateEvent) {
        elFormItem.formItemMitt?.emit("e.form.change", [actualValue.value]);
      }
    });
